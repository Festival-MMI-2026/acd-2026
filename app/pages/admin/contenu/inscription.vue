...
const { data: content, refresh } = await useFetch("/api/inscription-content");
...
async function save() {
  saving.value = true;
  try {
    await $fetch("/api/inscription-content", {
      method: "PUT",
      body: form.value,
    });
    await refresh();
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold tracking-tight">Page Inscription</h1>
        <p class="text-muted-foreground">
          Modifier le titre et la description de la page Inscription
        </p>
      </div>
    ...
    <Card class="rounded-2xl">
      <CardHeader>
        <div class="flex items-center gap-3">
          ...
          >
            <Icon name="lucide:clipboard-list" class="h-5 w-5 text-primary" />
          </div>
          <div>
            <CardTitle>En-tête de la page</CardTitle>
            <CardDescription>
              Le titre et la description affichés en haut de la page Inscription
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent class="space-y-5">
        ...
        <div class="space-y-2">
          ...
          <Textarea
            ...
            rows="2"
            placeholder="Inscrivez-vous pour participer..."
          />
        </div>
      </CardContent>
    </Card>
